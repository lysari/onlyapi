...
export const userHandlers = (userService: UserService, tokenService: TokenService, logger: Logger) => ({
  getMe: async (req: Request, ctx: RequestContext): Promise<Response> => {
    const authResult = await authenticate(req, tokenService);
    if (!authResult.ok) {
      logger.warn("Auth failed on getMe", { requestId: ctx.requestId, code: authResult.error.code });
  ...
  updateMe: async (req: Request, ctx: RequestContext): Promise<Response> => {
    const authResult = await authenticate(req, tokenService);
    if (!authResult.ok) {
      logger.warn("Auth failed on updateMe", { requestId: ctx.requestId, code: authResult.error.code });
      return errorResponse(authResult.error, ctx.requestId);
    }
    ...
    if (!validated.ok) {
      logger.warn("Update validation failed", { requestId: ctx.requestId, code: validated.error.code });
  ...
  deleteMe: async (req: Request, ctx: RequestContext): Promise<Response> => {
    const authResult = await authenticate(req, tokenService);
    if (!authResult.ok) {
      logger.warn("Auth failed on deleteMe", { requestId: ctx.requestId, code: authResult.error.code });
      return errorResponse(authResult.error, ctx.requestId);
    }
